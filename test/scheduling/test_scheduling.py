...
def test_avoid_cycle(orchestra: OrchestraShim):
    """Checks that orchestra is able to pick the correct dependency,
    even when the preferred dependency originates a cycle"""
...
def test_reject_choice_cycle(orchestra: OrchestraShim):
    """Checks that a choice which cannot be taken without generating an unsatisfied cycle
    raises an exception when installing and when printing the solved graph,
    but not when printing the unsolved graph"""
    # Print unsolved graph
...
def test_same_component_ordering(orchestra: OrchestraShim):
    """Checks that the edges added to enforce an order between installs
    of the same component do not introduce cycles"""
    # Print unsolved graph
    print("Unsolved graph")
