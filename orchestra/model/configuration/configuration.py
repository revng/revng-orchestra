...
class Configuration:
    ...
    def _check_minimum_version(self):
        ...
        if min_version:
            ...
            if __parsed_version__ < parsed_min_version:
                logger.error(f"This configuration requires orchestra version >= {min_version}, you have {__version__}")
                exit(1)
    ...
    def orchestra_root(self):
        return self._user_paths.get(
            "orchestra_root", os.path.realpath(os.path.join(self.orchestra_dotdir, "..", "root"))
        )

    @property
    def source_archives(self):
        return self._user_paths.get(
            "source_archives", os.path.realpath(os.path.join(self.orchestra_dotdir, "source_archives"))
        )

    @property
    def binary_archives_dir(self):
        return self._user_paths.get(
            "binary_archives", os.path.realpath(os.path.join(self.orchestra_dotdir, "binary-archives"))
        )

    @property
    def tmproot(self):
        return self._user_paths.get("tmproot", os.path.realpath(os.path.join(self.orchestra_dotdir, "tmproot")))

    @property
    def sources_dir(self):
        return self._user_paths.get(
            "sources_dir", os.path.realpath(os.path.join(self.orchestra_dotdir, "..", "sources"))
        )

    @property
    def builds_dir(self):
        return self._user_paths.get("builds_dir", os.path.realpath(os.path.join(self.orchestra_dotdir, "..", "build")))
